...
SQL);
        ...
        $indexExists = (bool) $this->connection->fetchOne("SELECT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = current_schema() AND tablename = 'retail' AND indexname = 'idx_retail_type_path_kind')");
        if (!$indexExists) {
